...
** Aim 1: Comparison of PU.1 genomic binding across immune cells reveals cell type-specific roles in autoimmune disease <<one>>
...
- Gene ontology analysis, via
  [[https://github.com/jokergoo/rGREAT][GREAT]] - Transcription factor motif enrichment analysis, via Homer
  - Used to find potential cooperative binding with PU.1
- Connections to GWAS studies via RELI, a tool created in the Weirauch lab
...
** Aim 2: A Pipeline Manager for Bioinformatics Analysis <<two>>
...
before. The tool, which has been tentatively named "Piphany" would
facilitate the analysis in Aim 3. 

A more detailed description of Piphany is located at the [[piphany][end of this document]].
...
strain/VTR/Cell Type combination against the GFP control.
...
* Aim 2 (extended) <<piphany>>

Below is a description of the current plans for the pipeline manager
"Piphany". Any thoughts or feedback are very much appreciated!
...
1. Piphany should have at least all of the features that any alternative
   would have
2. Piphany should be considerably easier to learn and use than any alternative
3. Piphany should be easy to maintain and take less effort to develop
...
Rust is used for piphany's "backend" for a few reasons. The central
...
Here, Piphany would determine that the outputs requires =proc2=, that
...
Piphany models each "step" in a pipeline as a "process", which can
