...
⁠```
#set document(title: "Dissertation Pre-proposal", date: datetime(year: 2026, month: 5, day: 8))
#set text(lang: "en")
...
#heading(level: 1)[Commitee members] #label("org555d321")
...
#heading(level: 1)[Introduction] #label("org91e7430")
...
#heading(level: 1)[Aims] #label("org3c23d5c")
...
facilitate analysis.
#heading(level: 2)[Aim 1: Comparison of PU.1 genomic binding across immune cells reveals cell type\u{2d}specific roles in autoimmune disease ] #label("org7fb49bc")
...
#link("https://github.com/jokergoo/rGREAT")[GREAT] #footnote(link("https://github.com/jokergoo/rGREAT")) \u{2d} Transcription factor motif enrichment analysis, via Homer
#list(list.item[Used to find potential cooperative binding with PU.1])])#list(list.item[Connections to GWAS studies via RELI, a tool created in the Weirauch lab])#list(list.item[Identification of allele\u{2d}dependent binding behavior via MARIO, a tool
...
for review.
#heading(level: 2)[Aim 2: A Pipeline Manager for Bioinformatics Analysis ] #label("org73c7f20")
While performing the analysis for #link(label("org7fb49bc"))[Aim 1], it became clear that the
...
before. The tool, which has been tentatively named \u{22}Piphany\u{22} would
facilitate the analysis in Aim 3. 

A more detailed description of Piphany is located at the #link(label("org95befa5"))[end of this document].
#heading(level: 2)[Aim 3: A detailed understanding of the effect of HPV viral transcriptional regulators on human gene regulation] #label("org828bf1d")
Similar to #link(label("org7fb49bc"))[Aim 1], this aim would consist of bioinformatics analysis,
...
analyses from #link(label("org7fb49bc"))[Aim 1] may be done here, though must be adapted to fit the
data.

Currently, the analysis is in an exploratory phase, though input from
committee members is of course appreciated.
#heading(level: 3)[Preliminary Results] #label("orgb2be6f3")
The following is a table containing IDR conservative peak counts and
the number of differentially expressed genes for the generated HPV data
sets. It should be noted that missing values do not indicate missing
data, but values that are yet to be determined. Each count of DEGs
represents the significantly differentially expressed genes of the HPV
strain\u{2f}VTR\u{2f}Cell Type combination against the GFP control.

#let data = csv("summary_sheet.csv")
#csvtable(data)
#heading(level: 1)[Aim 2 (extended) ] #label("org95befa5")
Below is a description of the current plans for the pipeline manager
\u{22}Piphany\u{22}. Any thoughts or feedback are very much appreciated!
#heading(level: 2)[The problem] #label("org8b9df8b")
...
a different way. This is one of the problems that Piphany aims to solve,
...
pipeline developed in Piphany would work in both places.
...
because, as a cross\u{2d}disciplinary field, many developers do not have formal
...
cluster. Piphany would create a unified interface. The user would
specify the container, and Piphany would handle the rest.

The final problem that Piphany aims to solve is one of organization. An analysis
...
analysis performed in #link(label("org7fb49bc"))[Aim 1] required dozens if not hundreds of steps
...
Piphany would keep track of which steps have been modified and run only
the appropriate steps based on those changes.
#heading(level: 3)[Current Solutions] #label("org1d6d977")
...
complete #link(label("org7fb49bc"))[Aim 1], has a significant learning curve and subpar error
...
#heading(level: 2)[Piphany\u{27}s design] #label("org09a2ea4")
The design of Piphany is guided by 3 principles.

#enum(enum.item(1)[Piphany should have at least all of the features that any alternative
would have],
enum.item(2)[Piphany should be considerably easier to learn and use than any alternative],
enum.item(3)[Piphany should be easy to maintain and take less effort to develop
than the alternatives],
)
#heading(level: 3)[Piphany Lang] #label("org8155ffa")
...
considerable effort. If Piphany were to use a fully custom language, it
would violate design principle 3. To this end, Piphany uses a language
...
to Piphany easy to implement. The reason scheme is used here instead of
...
Rust is used for piphany\u{27}s \u{22}backend\u{22} for a few reasons. The central
...
(output!
 \u{22}my_results.txt\u{22} : proc2)")]) #label("org0bb4982")


Here, Piphany would determine that the outputs requires #raw("proc2"), that
...
Piphany pipelines could be more complicated, but this is a basic
definition that would take little time to learn, while still being
useful when creating analyses. 
#heading(level: 3)[Piphany\u{27}s model] #label("org332294a")
Piphany models each \u{22}step\u{22} in a pipeline as a \u{22}process\u{22}, which can
...
variable or expression defined in the scheme language. Piphany checks
...
Piphany runs the entire scheme script before running the pipeline. This
way, Piphany can determine if everything is in order in the pipeline
before running anything. The scheme script creates a directed a\u{2d}cyclic
graph (DAG) which Piphany then interprets into a particular order to run
the processes.

Piphany also keeps track of modifications to the derivations, so any
...
reevaluated. Piphany does this by creating a cache of each derivation
